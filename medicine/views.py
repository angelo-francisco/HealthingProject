...
def auth_doctor(request):
    ...
    if is_medico(request.user):
        ...
        if request.method == "GET":
            ...
            messages.add_message(request, constants.SUCCESS, "Médico cadastrado!",{'is_medico': is_medico(request.user)})
            return redirect(reverse("horarios"))
...
def horarios(request):
    if request.method == "GET":
        ...
        if medico.exists():
            context = {"medico": medico.first(), "horarios": horarios, 'is_medico': is_medico(request.user)}
...
def medico_consultas(request):
    ...
    return render(
        ...
        {"consultas_hoje": consultas_hoje, "consultas_restantes": consultas_restantes, 'is_medico': is_medico(request.user)},
    )
...
def ver_consultas(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        if consulta.status == "F" or consulta.status == "C":
            messages.add_message(request, constants.WARNING, 'Consulta já foi finalizada/cancelada!')
